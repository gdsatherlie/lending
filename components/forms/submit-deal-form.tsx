...
import { submitDealAction } from '@/app/actions';
...
function SubmitButton() {
  ...
  return (
    ...
    >
      {pending ? 'Submitting...' : 'Submit Your Deal'}
    </button>
  );
}
...
export function SubmitDealForm() {
  const [state, formAction] = useFormState(submitDealAction, initialState);

  return (
    <form className="space-y-10 rounded-sm border border-stone bg-white p-6 shadow-soft md:p-8" action={formAction} encType="multipart/form-data">
